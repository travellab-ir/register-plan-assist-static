...
نسخه‌ی `4.0.1` این پکیج (که پروژه‌ی اصلی بهش قفل شده بود) از npm حذف شده. این پکیج آپدیت شده به `^4.1.5` (آخرین نسخه‌ی موجود، فقط باگ‌فیکس، بدون تغییر API) و فایل‌های `package-lock.json` قدیمی (که نسخه‌ی خراب رو قفل کرده بودن) حذف شدن تا npm نسخه‌ی جدید رو تازه resolve کنه.
