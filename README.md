...
- **Root directory باید `Client` باشه، نه ریشه‌ی ریپو.** چون فایل `package.json` که دستور بیلد توش تعریف شده داخل `Client` هست.
- پوشه‌ی `Core` باید کنار `Client` (نه داخلش) بمونه، چون کد با مسیر نسبی `../Core` بهش رفرنس می‌ده. اگه ساختار پوشه‌ها رو به‌هم نریزی، Cloudflare خودش این مسیر رو درست پیدا می‌کنه.
- چون از `HashRouter` استفاده شده (آدرس‌ها به‌شکل `#/preplan-list`)، نیازی به تنظیم قانون Redirect برای SPA نیست — از همون اول درست کار می‌کنه.
- این نسخه کاملاً **دمو/نمایشی** هست: تغییراتی که کاربر توی UI میده (مثل اضافه‌کردن پرواز) واقعاً ذخیره نمی‌شن، چون سروری در کار نیست. برای دیدن ظاهر و رفتار کلی برنامه عالیه.
